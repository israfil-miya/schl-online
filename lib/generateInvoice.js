...
export default async function createInvoice(
  ...
  try {
    ...
    for (let i = 0; i <= contactDetails.customer.length - 1; i++) {
      let rowNeeded = Math.round(getTextWidth(contactDetails.customer[i]) / 210) || 1
      
      contactTableLoopEndIndex += rowNeeded
      console.log(rowNeeded, contactDetails.customer[i])
      customerContactRowNeeded.push({start: lastEnd, end: lastEnd+rowNeeded-1 })
      lastEnd += rowNeeded
    }
    ...
    contactDetails.vendor.forEach((data, i) => {
    ...
    contactDetails.customer.forEach((data, i) => {
      let indexRow = i + contactTableHeadingRow + 1
      let row = sheet.getRow(indexRow);
      row.height = 17
