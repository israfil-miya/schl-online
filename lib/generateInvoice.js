...
export default async function createInvoice(
  ...
  try {
    ...
    for (let i = 0; i <= contactDetails.customer.length - 1; i++) {
      contactTableLoopEndIndex += Math.round(getTextWidth(contactDetails.customer[i]) / 210) || 1
      console.log(Math.round(getTextWidth(contactDetails.customer[i]) / 210) || 1, contactDetails.customer[i])
    }

    let afterContactTableRowNumber = contactTableHeadingRow + 1 + (contactTableLoopEndIndex <= 5 ? contactDetails.vendor.length : contactTableLoopEndIndex)
    let afterBillTableRowNumber = afterContactTableRowNumber + 5;

    // CONTACT TABLE
    for (let i = 0; i <= (contactTableLoopEndIndex <= 5 ? contactDetails.vendor.length : contactTableLoopEndIndex); i++) {
      ...
      if (contactDetails.customer[i])
        addHeader(
          sheet,
          `E${indexRow}:H${indexRow}`,
          {
            richText: [
              {
                font: { bold: true },
    ...
    console.log("afterContactTableRowNumber", afterContactTableRowNumber)

    sheet.addConditionalFormatting({
      ref: `A${afterContactTableRowNumber}:H${afterContactTableRowNumber}`,
      rules: [
        {
          type: "expression",
          formulae: ["true"],
    ...
    billData.forEach((data, index) => {
      ...
      sheet.getCell(`F${index}`).numFmt = `${'"' + currencySymbol + '"'
        }#,##0.00;[Red]\-"AUD"#,##0.00`;
      sheet.getCell(`G${index}:H${index}`).numFmt = `${'"' + currencySymbol + '"'
        }#,##0.00;[Red]\-"AUD"#,##0.00`;
    ...
    addHeader(
      ...
      {
        formula: `SUM(E${afterContactTableRowNumber + 5}:E${afterBillTableRowNumber - 1})`,
        result: totalFiles,
      },
      {
        name: "Arial",
    ...
    addHeader(
      ...
      {
        formula: `SUM(G${afterContactTableRowNumber + 5}:H${afterBillTableRowNumber - 1})`,
        result: subtotal,
      },
      {
        name: "Arial",
    ...
    addHeader(
      ...
      {
        formula: `(G${afterBillTableRowNumber + 1}-G${afterBillTableRowNumber + 2
    ...
    sheet.getCell(
      `G${afterBillTableRowNumber + 4}:H${afterBillTableRowNumber + 4}`,
    ).numFmt = `${'"' + currencySymbol + '"'}#,##0.00;[Red]\-${'"' + currencySymbol + '"'
    }#,##0.00`;
