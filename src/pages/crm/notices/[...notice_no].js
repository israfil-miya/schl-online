        ...
        try {
            ...
            if (!notice_data.error) {
                ...
                if (notice_data.channel != "marketers") {
                    toast.error("The notice doesn't belong to this channel", { toastId: "error2" })
                    router.push('/')
                }
        ...
        <>
            ...
            <div className='notice container mt-5 mb-3'>
                <div className='notice-header'>
                    ...
                    <p className='fs-6 fw-medium text-body-secondary' >{isoDateToFormattedString(notice.updatedAt)}</p>
                </div>
                <div className='notice-body mt-2 mb-3' dangerouslySetInnerHTML={{ __html: notice.description }} />

                {notice.file_name ? <div className='file-download'>
                    <span className='fw-semibold'>Downloads: </span> <span onClick={handleFileDownload}
                        className='text-decoration-underline font-monospace downloadable-file'>{notice.file_name}</span>
                </div> : null}
            </div>
            <style jsx>{`
.downloadable-file {
